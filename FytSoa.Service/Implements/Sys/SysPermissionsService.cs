...
namespace FytSoa.Service.Implements
{
    ...
    public class SysPermissionsService : DbContext, ISysPermissionsService
    {
        ...
        public async Task<ApiResult<string>> ToRoleAsync(SysPermissions parm,bool status)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    {
                        //角色-菜单-按钮功能
                        SysPermissionsDb.Delete(m => m.BtnFunJson == parm.BtnFunJson && m.RoleGuid == parm.RoleGuid && m.MenuGuid==parm.MenuGuid && m.Types == 3);
                    }
                }                
            }
            catch (Exception ex)
        ...
        public async Task<ApiResult<string>> SaveAsync(SysPermissions parm)
        {
            ...
            {
                //根据角色查询菜单列表
                var menuList = Db.Queryable<SysPermissions>().Where(m=>m.RoleGuid==parm.RoleGuid && m.Types==1).ToList();
                //现有的
        ...
        /// </summary>
        /// <returns></returns>
        public Task<ApiResult<string>> SaveAuthorization(List<SysMenuDto> list, string roleGuid)
        {
            var res = new ApiResult<string>() { statusCode = (int)ApiEnum.Error };
            try
            {
