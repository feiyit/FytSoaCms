...
namespace FytSoa.Service.Implements
{
    ...
    /// 菜单功能实现
    /// </summary>
    public class SysBtnFunService : DbContext, ISysBtnFunService
    {
        /// <summary>
        /// 添加一条记录
        /// </summary>
        ...
        /// <returns></returns>
        public async Task<ApiResult<string>> AddAsync(SysBtnFun parm)
        {
        ...
        public async Task<ApiResult<string>> DeleteAsync(string parm)
        {
            ...
            try
            {
                ...
                var dbres = SysBtnFunDb.Delete(m => list.Contains(m.Guid));
                if (!dbres)
                {
                    res.statusCode = (int)ApiEnum.Error;
                    res.message = "删除数据失败~";
       ...
       /// <param name="parm"></param>
       /// <returns></returns>
        public async Task<ApiResult<SysBtnFun>> GetByGuidAsync(string parm)
        {
            var model = SysBtnFunDb.GetById(parm);
            var res = new ApiResult<SysBtnFun>
            {
                statusCode = 200,
                data = model ?? new SysBtnFun() { }
            };
            return await Task.Run(() => res);
        }
        ...
        /// <param name="parm"></param>
        /// <returns></returns>
        public async Task<ApiResult<string>> ModifyAsync(SysBtnFun parm)
        {
            var res = new ApiResult<string>() { data = "1", statusCode = 200 };
            try
            {                
                var dbres= SysBtnFunDb.Update(parm);
                if (!dbres)
                {
                    res.statusCode = (int)ApiEnum.Error;
                    res.message = "修改数据失败~";
